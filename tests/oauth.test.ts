...
describe("connect URL", () => {
  it("carries the four query parameters the server expects", () => {
    const url = new URL(
      buildConnectUrl(
        ...
        "studylife-streamdeck",
        "http://127.0.0.1:8785/callback",
        "st4te",
        "ch4llenge",
      ),
    );
    expect(url.pathname).toBe("/connect/client/studylife-streamdeck");
    expect(url.searchParams.get("redirect_uri")).toBe("http://127.0.0.1:8785/callback");
  ...
  it("does not produce a double slash when the instance URL has a trailing one", () => {
    const url = buildConnectUrl("https://studylife.example.com/", "c", "http://127.0.0.1:8785/callback", "s", "c");
...
describe("loopback redirect", () => {
  ...
  it("uses ports that do not collide with studylife-cli's 8765-8768 or studylife-vscode's 8775-8778", () => {
    for (const port of CANDIDATE_PORTS) {
      expect(port).toBeGreaterThan(8778);
    }
  });

  it("is the fixed four-port list this plugin is registered with", () => {
    expect(CANDIDATE_PORTS).toEqual([8785, 8786, 8787, 8788]);
  });
});
