// Pure half of the login round trip: no Stream Deck SDK and no sockets in here, so the parts that
// are easy to get subtly wrong (PKCE shape, state comparison, callback parsing) are unit-testable.
//
// Ported verbatim in shape from studylife-vscode's oauth.ts, which is itself the generic-flow port
// of studylife-cli's login.py. Keep the wire shapes identical to that reference - the server
 ...
 * four are registered once for "studylife-streamdeck" via studylife-developers, and differ from
 * studylife-vscode's 8775-8778 and studylife-cli's 8765-8768 so all three can be logged in at the
 * same time.
 */
export const CANDIDATE_PORTS = [8785, 8786, 8787, 8788] as const;

export const DEFAULT_CLIENT_ID = "studylife-streamdeck";
