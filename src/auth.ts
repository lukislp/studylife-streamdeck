// The SDK/socket half of the login: binds a loopback listener, sends the user to the browser, and
// waits for the redirect, exchanging the one-time assertion for this installation's API key. The
// shapes it speaks are in oauth.ts and mirror studylife-vscode's auth.ts (and studylife-cli's
// login.py before it).
    ...
    const server: Server = createServer((req, res) => {
      ...
      res.end(
        "<!doctype html><meta charset=utf-8><title>StudyLife</title>" +
          ...
          "<h1>Connected</h1><p>You can close this tab and go back to Stream Deck.</p>",
      );
      settle?.(params);
    });
...
export async function runLogin(
  ...
  try {
    ...
    await streamDeck.system.openUrl(connectUrl);
    ...
    if (!statesMatch(result.state, state)) {
      throw new LoginError(
        "Rejected the login callback: its state did not match what this plugin sent. Try " +
          "connecting again.",
      );
    }
    if (!result.assertion) {
